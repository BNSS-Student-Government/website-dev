...
const ProfileFormExtended = ({ firstNameSet, lastNameSet, positionSet }) => {
  const [file, setFile] = useState(null);
  const [firstName, setFirstName] = useState(firstNameSet);
  const [lastName, setLastName] = useState(lastNameSet);
  const [vision, setVision] = useState("");
  const [experience, setExperience] = useState("");
  const [additional, setAdditional] = useState("");
  const [videoLink, setVideoLink] = useState("");
  const [position, setPositon] = useState(positionSet);
  ...
  let dueDate = new Date("2024-04-20T23:59:59");
